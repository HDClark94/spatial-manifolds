...
print(f"Running XGBoost Cell Number Assay for Mouse {mouse} Day {day} on cells {cell_start} to {cell_end-1} (history={history_length}ms, nfilters={nfilters})")
...
def _cell_out(tid):
    return f'{data_path}/xgboost_cell_number_assay_extra_M{mouse}_D{day}_h{history_length}_C{int(tid)}.csv'
...
for idx, id in enumerate(target_cells_batch):
    ...
    # --- Baselines: null + all covariate combos of pos / speed / lfp ---
