...
n_neurons = np.insert(n_neurons, 0, 0) # we want the condition where no cells are used as a covariate history as well

pR2s_grids_comodular = np.zeros((16, len(n_neurons), len(grid_module_population_cluster_ids))); pR2s_grids_comodular[:,:,:] = np.nan
pR2s_grids_non_comodular = np.zeros((16, len(n_neurons), len(grid_non_module_population_cluster_ids))); pR2s_grids_non_comodular[:,:,:] = np.nan
pR2s_non_grids = np.zeros((16, len(n_neurons), len(non_grid_population_cluster_ids))); pR2s_non_grids[:,:,:] = np.nan
...
for test_population_cluster_ids, pR2s, label in zip([grid_module_population_cluster_ids, grid_non_module_population_cluster_ids, non_grid_population_cluster_ids],
    ...
    for j, n in enumerate(n_neurons):
        ...
        for i, id in enumerate(test_population_cluster_ids):
            print(f'{np.sum(~np.isnan(pR2s_grids_comodular))}/{np.size(pR2s_grids_comodular)}, {np.sum(~np.isnan(pR2s_grids_non_comodular))}/{np.size(pR2s_grids_non_comodular)}, {np.sum(~np.isnan(pR2s_non_grids))}/{np.size(pR2s_non_grids)}')
            ...
            # randomise the order of the covariate cells so that we are not always taking the nearest or farthest cells first, but instead a random selection of cells       
    ...
    # Save
    with open(f'{data_path}/xgboost_{assay_mode}_cell_number_assay_M{mouse}_D{day}_{label}.yaml', 'w') as f:
