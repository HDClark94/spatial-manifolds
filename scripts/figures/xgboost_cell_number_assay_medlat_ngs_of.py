...
def _cell_out(tid):
    return f'{data_path}/xgboost_medlat_ngs_of_M{mouse}_D{day}_h{history_length}_C{int(tid)}.csv'
...
for idx, id in enumerate(target_cells_batch):
    ...
    try:
        theta = get_theta_trace(
            ...
            time_bs=50, resample_bs=time_bs, session_type='OF1', source_path=source_path,
        )
