"""Per-net within ΔpR² + cross ΔpR² from the (partial) nets-6+7 cache. net_id 0=net6, 1=net7."""
import numpy as np
c=np.load('/Users/harryclark/Documents/spatial-manifolds/data/rnn_xgboost/pr2_two_net_67_cache.npz', allow_pickle=True)
...
print(f"{int(completed.sum())}/{N} done | net6 within={mn(w[0]):+.4f} | net7 within={mn(w[1]):+.4f} | cross={mn(cross):+.4f}")
