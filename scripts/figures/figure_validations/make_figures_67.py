...
c=np.load(DATA+'/pr2_two_net_67_cache.npz',allow_pickle=True)
...
fig.suptitle('Two-network control — nets 6 & 7 (grid-forming 1024 seeds)',fontsize=11,fontweight='bold')
plt.tight_layout(rect=[0,0.03,1,0.96])
plt.savefig(FIG+'/two_network_67_matrices.pdf',bbox_inches='tight'); plt.savefig(FIG+'/two_network_67_matrices.png',dpi=120,bbox_inches='tight'); plt.close()
...
plt.tight_layout(); plt.savefig(FIG+'/two_network_67_control.pdf',bbox_inches='tight'); plt.savefig(FIG+'/two_network_67_control.png',dpi=120,bbox_inches='tight'); plt.close()
...
print(f'=== COMPARISON of cross-net ΔpR² floor: pretrained4096=0.0106 (1.66x) | sharedPC-1024=0.0245 (1.01x) | nets6+7={xm:.4f} ({wm/max(xm,1e-9):.2f}x)', flush=True)
