...
CACHE=DATA+'/pr2_two_net_67_cache.npz'
...
print(f'netA(net6:4444/pc44) {len(np.where(sc1>GC_THRESH)[0])} GCs, random {len(sel1)} selected, '
...
def save():
    np.savez(CACHE,pr2_base=pr2_base,pr2_cell=pr2_cell,pr2_full=pr2_full,completed=completed,
             ...
             base_cols=np.array(['pos_x','pos_y','speed','cos_hd','sin_hd']),net_seeds=np.array([4444,5555]))
save()
