"""Generate the DIFFERENT-place-cell two-network control figures + compare cross-floors."""
...
c=np.load(DATA+'/pr2_two_net_diffPC_cache.npz',allow_pickle=True)
...
for ax,M,title,cmap,lo,hi in [
    ...
    im=ax.imshow(M,cmap=cm,vmin=lo,vmax=hi,interpolation='nearest'); blocks(ax,title); fig.colorbar(im,ax=ax,fraction=0.046,pad=0.02)
fig.text(0.5,0.02,'rows = target | cols = source | base = position + speed + head direction | grey = self or excluded (pr2_base<0.3)',ha='center',fontsize=8,color='#555')
fig.suptitle('Two-network control — DIFFERENT place-cell tilings (two from-scratch 1024-unit nets)',fontsize=11,fontweight='bold')
plt.tight_layout(rect=[0,0.03,1,0.96])
plt.savefig(FIG+'/two_network_diffPC_matrices.pdf',bbox_inches='tight'); plt.savefig(FIG+'/two_network_diffPC_matrices.png',dpi=120,bbox_inches='tight'); plt.close()
...
plt.tight_layout(); plt.savefig(FIG+'/two_network_diffPC_control.pdf',bbox_inches='tight'); plt.savefig(FIG+'/two_network_diffPC_control.png',dpi=120,bbox_inches='tight'); plt.close()

wm,xm=dpr2[within].mean(),dpr2[~within].mean()
print('FIGURES_DONE', flush=True)
print(f'=== diffPC RESULT: within ΔpR²={wm:.4f} cross={xm:.4f} ratio={wm/max(xm,1e-9):.2f}x | '
      f'Spearman(|J|,ΔpR²)={r_d:+.3f} Spearman(|J|,pR²cell)={r_c:+.3f} | strong-within={dpr2[hi].mean()/max(floor,1e-9):.1f}x floor', flush=True)
print(f'=== COMPARISON of cross-net ΔpR² floor: pretrained4096=0.0106 (1.66x) | sharedPC-1024=0.0245 (1.01x) | diffPC-1024={xm:.4f} ({wm/max(xm,1e-9):.2f}x)', flush=True)
