...
NETA=DATA+'/diffPC_A_Ng1024'; NETB=DATA+'/diffPC_B_Ng1024'
CACHE=DATA+'/pr2_two_net_diffPC_cache.npz'
...
print(f'netA(seed1234,pc101) {len(np.where(sc1>GC_THRESH)[0])} GCs, random {len(sel1)} selected, '
      f'mean|J|={np.abs(Jf1[np.ix_(sel1,sel1)]).mean():.4f} | '
      f'netB(seed5678,pc202) {len(np.where(sc2>GC_THRESH)[0])} GCs, random {len(sel2)} selected, '
...
def save():
    np.savez(CACHE,pr2_base=pr2_base,pr2_cell=pr2_cell,pr2_full=pr2_full,completed=completed,
             ...
             base_cols=np.array(['pos_x','pos_y','speed','cos_hd','sin_hd']),net_seeds=np.array([1234,5678]))
save()
