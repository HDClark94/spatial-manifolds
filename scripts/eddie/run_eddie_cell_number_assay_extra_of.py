...
for mouse, days in mouse_days.items():
    for day in days:
        ...
        data_path = f"/exports/eddie/scratch/hclark3/data/xgboost_cell_number_assay_of/"
        stageout_dict = {
            data_path: '/exports/cmvm/datastore/sbms/groups/CDBS_SIDB_storage/NolanLab/ActiveProjects/Harry/SpatialLocationManifolds2025/data/xgboost_cell_number_assay_of/'
        }
        for batch_idx in range(n_batches):
            cell_start = batch_idx * BATCH_SIZE
            cell_end = min((batch_idx + 1) * BATCH_SIZE, n_cells)
            job_name = f"M{mouse}D{day}XOF_{cell_start}_{cell_end}"
            run_python_script(
                f"/exports/eddie/scratch/hclark3/spatial-manifolds/scripts/figures/xgboost_cell_number_assay_extra_of.py --mouse={mouse} --day={day} --data_path={data_path} --cell_start={cell_start} --cell_end={cell_end}",
