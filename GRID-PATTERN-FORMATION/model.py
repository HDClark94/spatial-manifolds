...
class RNN(torch.nn.Module):
    ...
    def set_weights(self, weights):
        w0, w1, w2, w3 = weights
        with torch.no_grad():
            self.encoder.weight.copy_(torch.from_numpy(w0.T).float())
            self.RNN.weight_ih_l0.copy_(torch.from_numpy(w1.T).float())
            self.RNN.weight_hh_l0.copy_(torch.from_numpy(w2.T).float())
            self.decoder.weight.copy_(torch.from_numpy(w3.T).float())
